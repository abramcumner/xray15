...
template	<typename K, class V, class P=std::less<K>, typename allocator = xalloc<std::pair<const K,V> > >	class	xr_multimap		: public std::multimap<K,V,P,allocator>	{ public: u32 size() const {return (u32)__super::size(); } };
	...
	template	<typename K, class _Tr = stdext::hash_compare<K, std::less<K> >, typename allocator = xalloc<K> > class xr_hash_set : public stdext::hash_set<K, _Tr, allocator> { public: u32 size() const { return (u32)__super::size(); } };
#endif // #ifdef STLPORT
