...
void CUIDosimeter::construct(CDosimeter* p)
{
	...
	xml_init.InitWindow(uiXml, "dosimeter:wrk_area", 0, m_wrk_area);
...
void CUIDosimeter::update()
{
	...
	rad *= 1000;
	if (rad > 999.0f) rad = 999.0f;
	string16 s;
	sprintf_s(s, "%04.0lf", rad);
	string16 tex;
	sprintf_s(tex, "seg7_digit%c", s[1]);
	m_seg1->InitTexture(tex);
	sprintf_s(tex, "seg7_digit%c", s[2]);
	m_seg2->InitTexture(tex);
	sprintf_s(tex, "seg7_digit%c", s[3]);
	m_seg3->InitTexture(tex);
